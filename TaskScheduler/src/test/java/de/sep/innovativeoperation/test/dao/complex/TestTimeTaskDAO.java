...
public class TestTimeTaskDAO {
	...
	public void setUp() {
		...
		timeTaskNullFirstFireTime.setFirstFireTime(calendar);
		...
		timeTaskNullIntervall.setFirstFireTime(null);
	...
	@Test
	public void testSaveTimeTasksWithNullValuesTimeTaskDAO() {
		TimeTask savedTimeTask = timeTask;
		try {
			savedTimeTask = timeTaskDAO.save(timeTaskNullFirstFireTime);
		} catch (Exception e) {
		...
		try {
			savedTimeTask = timeTaskDAO.save(timeTaskNullIntervall);
		} catch (Exception e) {
		...
		savedTimeTask = timeTask;
		try {
			savedTimeTask = timeTaskDAO.save(timeTaskNullIssueDrafts);
		} catch (Exception e) {
			System.out.println("saveTimeTaskNullIssueDrafts Exception!");
		}
		assertTrue(savedTimeTask.getId() > 0);
		assertNotNull(timeTaskDAO.findById(savedTimeTask.getId()));
		...
		savedTimeTask = timeTask;
		try {
			savedTimeTask = timeTaskDAO.save(timeTaskNullName);
		} catch (Exception e) {System.out.println("timeTaskNullName Exception!");}
